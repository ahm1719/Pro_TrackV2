  ...
  useEffect(() => {
    if (isSyncEnabled) {
      const unsubscribe = subscribeToCollections({
          onTasks: (cloudTasks) => {
              setTasks(prev => {
                  // IF cloud is empty AND we have local data AND we haven't synced yet
                  // THEN the cloud state might be unreliable/new, so we push local to cloud
      ...
      case ViewMode.TASKS:
        return (
          <div className="h-full flex flex-col space-y-6 animate-fade-in">
             <div className="flex justify-between items-center"><h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Daily Tasks</h1><div className="flex gap-3"><button onClick={() => setExpandedDay(todayStr)} className="flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 px-4 py-3 rounded-xl hover:border-indigo-300 dark:hover:border-indigo-500 font-bold shadow-sm transition-all"><Maximize2 size={18} /> Focus Today</button><button onClick={() => setShowNewTaskModal(true)} className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-xl hover:bg-indigo-700 transition-all shadow-lg font-bold"><Plus size={20} /> New Task</button></div></div>
